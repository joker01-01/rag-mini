...
对个人知识库来说，RAG 是性价比最高的方案。但市面上的 RAG 方案大多绕不开两个字——"重"：要么是十几个组件的完整框架，文档解析、向量库、重排序、Agent、评估一个不少，光看架构图就想放弃；要么强调需要 GPU、需要服务器，个人用户直接被劝退。

所以我想了个最简的 RAG 思路：把流程拆到不能再拆——转换、切分、向量化、检索、生成，五个环节全部用开源工具加一个 API 完成。不需要 GPU，不需要服务器，一台普通电脑就够了。整个数据流是这样的：
...
整套下来，除了 API 调用费，几乎没有成本。完整可运行的代码放在 [rag-mini 工程](https://github.com/joker01-01/rag-mini)，核心逻辑就是上面这几段。
